...
class Window(tk.Toplevel):
    ...
    def __init__(self, parent):
        ...
        #pour eviter des erreur eventuelles pendant l'Execution on utilise try
        try:
            cursor = db.data.cursor()
            cursor.execute("SELECT DISTINCT nom_region FROM Regions ORDER BY nom_region")
            regions = [row[0] for row in cursor.fetchall()]
        except Exception as e:
            print("Error fetching regions:", repr(e))
            regions = []


        #TODO Q3 C'est cette partie que l'on souhaite changer pour un choix dynamique de la région
        self.combobox = ttk.Combobox(self, values=regions, state='readonly')
        self.combobox.grid(row=1, column=1)
        self.combobox.bind('<Return>', self.searchRegion) # On bind l'appui de la touche entrée sur la case de saisie, on peut donc utiliser soit la touche entrée soit le bouton valider
